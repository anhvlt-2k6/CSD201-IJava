...
//        }
    ...
    void spDijkstra(int start, int des) {
        ...
        if (distance[des] == Integer.MIN_VALUE || distance[des] == Integer.MAX_VALUE) {
            resultspDijkstra += "," + des;
        }
        
        //resultspDijkstra += start + " " + des + ": " + distance[des] + "\n";
    }
...
//            out.write(resultDijkstra);
            //In SP va Path giua hai dinh
            if (resultspDijkstra.length() == 0) {
                out.write("Operational Dispatch System");
            } else {
                out.write(resultspDijkstra.substring(1));
            }
...
//                int w = sc.nextInt(); // Nếu có trọng số
                ...
                graph[to][from] = 1; // Vì đồ thị vô hướng
//                graph[from][to] = w; // Nếu có trọng số
//                graph[to][from] = w; // Nếu đồ thị vô hướng có trọng số           
            }
