...
//                    temp = current.left.findMaxNode();
    ...
    void findAncestors(BSTNode currentNode, int target) {
        if (currentNode == null) {
            return;
        }
        ...
        if (target < currentNode.data) {
            for (int i = 0; i < currentNode.count; i++) 
                resultAncestors += "," + currentNode.data;
            findAncestors(currentNode.left, target);
        } else if (target > currentNode.data) {
            for (int i = 0; i < currentNode.count; i++) 
                resultAncestors += "," + currentNode.data;
            findAncestors(currentNode.right, target);
        } else {
            return; // tìm thấy node
        }
    }
...
//            System.out.println("null");
            out.write(resultAncestors.substring(1));
            out.close();
